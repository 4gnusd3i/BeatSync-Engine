...
from ..application.path_utils import normalize_local_path, recommend_output_filename as backend_recommend_output_filename
from ..application.render_service import process_render
from ..application.source_service import inspect_sources
from ..application.startup_service import cleanup_on_startup as backend_cleanup_on_startup
from ..domain.models import RenderRequest
from ..processing.ffmpeg import DEFAULT_STANDARD_QUALITY, FFMPEG_FOUND, FFMPEG_PATH, check_nvenc_support, create_lossless_delivery_mp4, get_video_fps, get_video_resolution, is_browser_playable_video, normalize_quality_profile
from ..processing.renderer import CPU_COUNT, MAX_THREADS, PARALLEL_WORKERS, create_music_video, estimate_threads_per_job, get_local_temp_dir, get_video_files
from ..analysis.manual import analyze_beats_manual, process_manual_intensity
from ..analysis.smart import analyze_beats_smart, get_gpu_info, get_preset_info, is_gpu_available, select_beats_smart, set_gpu_mode
...
def build_source_summary(audio_path: str, video_folder: str) -> str:
    inspection = inspect_sources(audio_path, video_folder)
    cards: List[str] = []

    if not inspection.normalized_audio_path and not inspection.normalized_video_folder:
        return f"""
    ...
    cards.append(
        build_summary_card(
            "Audio track",
            inspection.audio_title,
            inspection.audio_detail,
            tone=inspection.audio_state,
        )
    )
    cards.append(
        build_summary_card(
            ...
            inspection.video_detail,
            tone=inspection.video_state,
        )
    )
...
def recommend_output_filename(current_filename: str | None, processing_mode: str) -> str:
    return backend_recommend_output_filename(current_filename, processing_mode)
...
def process_video(audio_path: str, video_folder: str, generation_mode: str, cut_intensity: float, smart_preset: str, output_filename: str, direction: str, playback_speed_str: str, timing_offset: float, parallel_workers: int, processing_mode: str, standard_quality: str, create_prores_delivery_mp4: bool | str | None, custom_resolution: str, custom_fps: float, session_state: dict) -> StatusResult:
    ...
    result = process_render(
        RenderRequest(
            ...
            direction=direction,
            playback_speed=playback_speed_str,
    ...
    return result.preview_path, result.status_text, session_state


def cleanup_on_startup():
    backend_cleanup_on_startup()
