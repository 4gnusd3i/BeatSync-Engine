...
from ..domain.presets import SMART_PRESETS_CONFIG as SMART_PRESETS
from ..processing.ffmpeg import detect_video_scene_changes
...
def _probe_gpu_support() -> tuple[object | None, bool, str | None]:
    try:
    ...
    try:
        device_count = cp_module.cuda.runtime.getDeviceCount()
        if device_count < 1:
            raise RuntimeError("No CUDA devices detected")
        return cp_module, True, None
    except Exception as exc:
        return cp_module, False, str(exc)


cp, GPU_AVAILABLE, GPU_INIT_ERROR = _probe_gpu_support()
