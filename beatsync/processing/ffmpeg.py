...
FFMPEG_PATH = str(REPO_ROOT / "bin" / "ffmpeg" / "ffmpeg.exe")
FFPROBE_PATH = str(REPO_ROOT / "bin" / "ffmpeg" / "ffprobe.exe")
...
def _initialize_ffmpeg_environment(announce: bool = False) -> tuple[str, str, str, bool]:
    portable_ffmpeg = str(REPO_ROOT / "bin" / "ffmpeg" / "ffmpeg.exe")
    portable_ffprobe = str(REPO_ROOT / "bin" / "ffmpeg" / "ffprobe.exe")
    ffmpeg_dir = os.path.dirname(portable_ffmpeg)

    if os.path.exists(portable_ffmpeg):
        prepend_path_once(ffmpeg_dir)
        os.environ["IMAGEIO_FFMPEG_EXE"] = portable_ffmpeg
        os.environ["FFMPEG_BINARY"] = portable_ffmpeg
        if os.path.exists(portable_ffprobe):
            os.environ["FFPROBE_BINARY"] = portable_ffprobe
        if announce:
            print(f"Using portable FFmpeg: {portable_ffmpeg}")
        return portable_ffmpeg, portable_ffprobe, ffmpeg_dir, True

    if announce:
        print(f"Portable FFmpeg not found at: {portable_ffmpeg}")
        print("   Falling back to system FFmpeg")
    return "ffmpeg", "ffprobe", ffmpeg_dir, False


FFMPEG_PATH, FFPROBE_PATH, FFMPEG_DIR, FFMPEG_FOUND = _initialize_ffmpeg_environment()
